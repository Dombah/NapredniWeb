                ...
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('tasks.store') }}">
                        ...
                        <div class="mb-4">
                            ...
                            <input id="naziv_rada" type="text" name="naziv_rada" value="{{ old('naziv_rada') }}" required autofocus
                        ...
                        <div class="mb-4">
                            ...
                            <input id="naziv_rada_eng" type="text" name="naziv_rada_eng" value="{{ old('naziv_rada_eng') }}" required
                                class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                        ...
                        <div class="mb-4">
                            ...
                            <select id="tip_studija" name="tip_studija" required
                                ...
                                <option value="stručni" {{ old('tip_studija') === 'stručni' ? 'selected' : '' }}>{{ __("tasks.Professional") }}</option>
                                <option value="preddiplomski" {{ old('tip_studija') === 'preddiplomski' ? 'selected' : '' }}>{{ __("tasks.Undergraduate") }}</option>
                                <option value="diplomski" {{ old('tip_studija') === 'diplomski' ? 'selected' : '' }}>{{ __("tasks.Graduate") }}</option>
                            </select>
                            @error('tip_studija')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="zadatak_rada" name="zadatak_rada" rows="8" required
                                class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('zadatak_rada') }}</textarea>
                            @error('zadatak_rada')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
